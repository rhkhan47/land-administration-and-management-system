...
function isAuthority() {
    return isset($_SESSION["user_type"]) && $_SESSION["user_type"] === "Authority";
}
...
function ensureUserActiveOnChain($link, $user_id) {
    ...
    try {
        ...
        if (strpos($msg, 'does not exist') !== false) {
            $row = null;
            if ($stmt = mysqli_prepare($link, "SELECT national_id, name, dob, permanent_address FROM users WHERE id = ?")) {
            ...
            if ($row) {
                fabric_invoke('registerUser', [$chaincodeUserId, $row['national_id'], json_encode(['name' => $row['name'], 'dob' => $row['dob'], 'address' => $row['permanent_address']])]);
                fabric_invoke('approveUser', [$chaincodeUserId]);
            }
        } else if (
            strpos($msg, 'DiscoveryService') !== false ||
            strpos($msg, 'failed constructing descriptor') !== false ||
            strpos($msg, 'Fabric API request failed') !== false ||
            strpos($msg, 'ECONNREFUSED') !== false ||
            strpos($msg, 'HTTP') !== false
        ) {
            return;
        } else {
            return;
...
function getUserLands($link, $user_id, $status = null) {
    ...
    $sql = "SELECT la.*, (
            ...
            FROM land_assets la WHERE la.owner_id = ?";
    if ($status) {
        $sql .= " AND la.status = ?";
    }
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
        if ($status) {
            mysqli_stmt_bind_param($stmt, "iis", $user_id, $user_id, $status);
        } else {
